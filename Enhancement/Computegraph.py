...
def Newword_compute(category):
	try:
		# Adds new edges if node already present in graph
		global hash1, hash2, hash3, hash4, count_synsets, start_time
		...
		for key, value in hash2.items():
			if value.category() == category: # Only for wordnet processing
				count_synsets += 1
				synset = Synfactory(value)
				
				print 'New Synset Added into Hash4 :',key
				# Enter Into Hash4
				hash4[key] = synset

				# Just process Words now
				Process_words(value, synset['S2D'], synset['S2E'])
		raise StopIteration('New Entries Computed')
...
if __name__ == '__main__':
	...
	try:
		for key, value in hash2.items():
			if value.category() == 'wordnet': # Only for wordnet processing
				count_synsets += 1
				synset = Synfactory(value)
