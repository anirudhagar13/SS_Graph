...
def E2S(src, data):
    '''
    To create word to sense examples backedges in which it occurs
    '''
    for dest in data.keys():
        edge = Edge(src=src, dest=dest, kind='E2S')
        # No need to populate edge as default weight = 0, which will prevent traversal
...
def Newword_graph(wordnodes, synsetnodes):
    '''
    To create new entries for words and synsets
    '''
    global hash3, hash4, graph
    ...
    try:
        ...
        for key in wordnodes:
            ...
            if key not in graph:    # Not changing existing entries
                print 'New Word being Added in graph :',key
                graph[key] = list()
                Words(key, hash3[key])
                word_count += 1
        for key in synsetnodes:
            #Initialize empty list for each entry
            if key not in graph:
                print 'New Synset being Added in graph :',key
                graph[key] = list()
                Synsets(key, hash4[key])
                synset_count += 1
        end_time = time.time()
        print 'Graph Creation Time : ',end_time - start_time
        raise StopIteration('New Entries Added in graph')
...
if __name__ == '__main__':
    ...
    # Showgraph(graph, 'chemical_formula')   # To show all edges of a word/synset
    try:
        start_time = time.time()
