...
def defn_word_to_sense_processing(synset_defn,synset,wd_filename,tokens_1,tokens_2):
    ...
    hash1.close()
'''
def defn_word_to_sense_processing(synset_defn, synset, wd_filename):
    try:
        word_data = shelve.open(wd_filename, writeback='True')
    except:
        print 'Cannot open word_data'

    tokens_1 = [x.lower() for x in synset_defn.split()] #single words
    ...
    for word in tokens_1 + tokens_2:
        if word_data.has_key(word):
            if word_data[word].has_key('D2S'):
                #word already exists in some sense defn
                word_data[word]['D2S'][synset] = count_words(word, synset_defn)
            else:
                #word not already in any sense defn
                word_data[word]['D2S'] = dict()
                word_data[word]['D2S'][synset] = count_words(word, synset_defn)
        else:
            word_data[word] = dict()
...
if __name__ == "__main__":
    ...
    sense_processing(sd_filename='Shelves/Hash#4-debug.shelve', wd_filename='Hash#3.shelve')
